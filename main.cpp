...
public:
    ...
    float get_p_bolt() { return P_BOLT; } // تم تصحيح الاسم هنا بالملي
...
int main() {
    ...
    // 1️⃣ الصفحة الرئيسية الحالية للحاسبة الفخمة
    ...
    svr.Get("/calculate", [&elevator](const httplib::Request& req, httplib::Response& res) {
        ...
        float c_bolts = bolts * elevator.get_p_bolt(); // تم تعديلها هنا برضه لـ bolt
    ...
    // 3️⃣ مسار المقالات المستقبلي الجديد (مؤمن وبدون تداخل)
    svr.Get("/blog", [](const httplib::Request&, httplib::Response& res) {
        string blog_html = "<html><head><meta charset='UTF-8'><meta name='viewport' content='width=device-width, initial-scale=1.0'>"
                           ...
                           "<a class='btn-back' href='/'>🧮 العودة للحاسبة الذكية</a>"
                           "</div></body></html>";
